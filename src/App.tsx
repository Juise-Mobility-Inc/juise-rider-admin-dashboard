...
import { type PackMapMarker, type PackMapPoint } from "./components/PackLocationPicker";
...
const defaultManagedAppId = import.meta.env.VITE_DEFAULT_MANAGED_APP_ID ?? "juise-customer-app";
...
function zoneToDraft(zone: SchoolZone): ZoneDraft {
        return {
                ...
                speed_limit_mph:
                        typeof zone.speed_limit_mph === "number" ? String(zone.speed_limit_mph) : "",
...
function createEmptyZoneDraft(zoneType: ZoneDraft["zone_type"] = "no_go"): ZoneDraft {
        return {
                id: makeDraftId(),
                zone_uuid: "",
                title: "",
...
function getScavengerHuntMinAccuracy(challenge: Pick<SchoolChallenge, "game_config">): string {
        const value = challenge.game_config?.min_accuracy_meters;
        return typeof value === "number" && Number.isFinite(value) && value > 0 ? String(value) : "50";
}
...
function getScavengerHuntRequiredDwellSeconds(
        ...
        return typeof value === "number" && Number.isFinite(value) && value > 0 ? String(value) : "30";
}

function getScavengerHuntGrandPrizePoints(challenge: Pick<SchoolChallenge, "game_config">): string {
...
function challengeToDraft(challenge: SchoolChallenge): ChallengeDraft {
        ...
        return {
                ...
                audience_type: challengeType === "scavenger_hunt" ? "user" : challenge.audience_type,
                title: challenge.title,
                description: challenge.description,
                image_url: challenge.image_url,
                metric_type: challengeType === "scavenger_hunt" ? "points" : challenge.metric_type,
                target_value:
...
function challengeToResubmitDraft(challenge: SchoolChallenge): ChallengeDraft {
        ...
        const durationSeconds = Math.max(60 * 60, challenge.end_time - challenge.start_time);
...
function getCreatedChallenges(
        response:
                SchoolChallenge | { challenge: SchoolChallenge; repeated_challenges?: SchoolChallenge[] },
): SchoolChallenge[] {
...
function sortChallengesForDisplay(challenges: SchoolChallenge[]): SchoolChallenge[] {
...
function resolveSectionFromPathname(pathname: string): Section | null {
        ...
        const matchingSection = dashboardSections.find(({ path }) => path === normalizedPath);
...
function App() {
        ...
        const [initialSession] = useState<AdminSession | null>(() => readDashboardSession());
        const [session, setSession] = useState<AdminSession | null>(null);
        const [authInitializing, setAuthInitializing] = useState(() => initialSession !== null);
        const [context] = useState<DashboardContext>(() => readDashboardContext(defaultManagedAppId));
        ...
        const [challengeParticipantsBusy, setChallengeParticipantsBusy] = useState(false);
        ...
        const [packEditDraft, setPackEditDraft] = useState<PackEditDraft | null>(null);
        ...
        const [selectedStudentDeviceUUID, setSelectedStudentDeviceUUID] = useState<string | null>(null);
        ...
        const packsWithoutLocationsCount = schoolPacks.length - existingPackMapMarkers.length;
        ...
        const sortedSchoolStudentRoster = useMemo(
                () =>
                        [...schoolStudentRoster].sort((left, right) => {
                                ...
                                return left.membership.student_id.localeCompare(right.membership.student_id);
                        }),
                [schoolStudentRoster],
        );
        ...
        const selectedStudentDeviceMediaAssets = useMemo(
                () =>
                        selectedStudentDevice
                                ? (studentDeviceMediaByDevice[selectedStudentDevice.registered_device_uuid] ?? [])
                                : [],
        ...
        const schoolReservationsByMembership = useMemo(() => {
                ...
                for (const reservation of schoolStudentReservations) {
                        ...
                        const currentReservations = reservationsByMembership.get(membershipUUID) ?? [];
        ...
        const challengeParticipantSummary = useMemo(
                () => ({
                        ...
                        active: challengeParticipants.filter((participant) => participant.active).length,
                        completed: challengeParticipants.filter((participant) => participant.completed).length,
                }),
        ...
        function handleDownloadPackQrCode(targetPack: Pack) {
                ...
                triggerFileDownload(
                        getAdminPackQrCodeDownloadUrl(session.claims.user_uuid, targetPack.pack_uuid),
                );
        }
        ...
        function handleDownloadPackSpotQrCode(spot: PackSpot) {
                ...
                triggerFileDownload(
                        getAdminPackSpotQrCodeDownloadUrl(session.claims.user_uuid, spot.spot_uuid),
                );
        }
        ...
        useEffect(() => {
                ...
                setActivePoiDraftId((current) => {
                        ...
                        return poiDrafts.some((poi) => poi.id === current) ? current : poiDrafts[0].id;
                });
        }, [poiDrafts]);
        ...
        useEffect(() => {
                ...
                setActiveZoneDraftId((current) => {
                        ...
                        return zoneDrafts.some((zone) => zone.id === current) ? current : zoneDrafts[0].id;
                });
        }, [zoneDrafts]);
                ...
                async function refreshStoredSession() {
                        ...
                        try {
                                ...
                                setBanner({
                                        tone: "info",
                                        message:
                                                getErrorMessage(error) || "Your session has expired. Please sign in again.",
                                });
                        } finally {
                                if (!cancelled) {
                                        setAuthInitializing(false);
        ...
        function handleOpenImagePreview(imageUrl: string, alt: string, label?: string) {
        ...
        async function refreshSchoolChallenges() {
                ...
                setChallengeListBusy(true);
                try {
                        const challenges = await fetchSchoolChallenges(context.managedAppId, activeSchoolId);
                        setSchoolChallenges(sortChallengesForDisplay(challenges));
                } catch (error) {
                        setBanner({
                                tone: "error",
        ...
        async function handleOpenStudentDeviceFromDashboard(membershipId: string, deviceUUID: string) {
                const normalizedMembershipId = membershipId.trim();
        ...
        async function handleSchoolLogoFileChange(event: ChangeEvent<HTMLInputElement>) {
                ...
                try {
                        const upload = await uploadSchoolLogoImage(context.managedAppId, activeSchoolId, file);
        ...
        async function handlePackPhotoFileChange(event: ChangeEvent<HTMLInputElement>) {
                const file = event.target.files?.[0];
                event.target.value = "";
        ...
        async function handlePackEditPhotoFileChange(event: ChangeEvent<HTMLInputElement>) {
                const file = event.target.files?.[0];
                event.target.value = "";
        ...
        async function handleSavePackEdit(event: FormEvent<HTMLFormElement>, pack: Pack) {
                event.preventDefault();
                ...
                try {
                        if ((trimmedLat && !trimmedLng) || (!trimmedLat && trimmedLng)) {
                                throw new Error("Provide both latitude and longitude to update the pack pin.");
                        }
                ...
                try {
                        const savedPOIs = await saveSchoolPOIs(
                                ...
                                nextPoiDrafts.map((poi, index) => {
                                        ...
                                        const lat = parseCoordinateInput(poi.lat, `POI ${index + 1} latitude`);
                                        const lng = parseCoordinateInput(poi.lng, `POI ${index + 1} longitude`);
                                        const bonusPoints = Number.parseInt(poi.bonus_points.trim(), 10);
                        ...
                        const savedZones = await saveSchoolZones(
                                ...
                                nextZoneDrafts.map((zone, index) => {
                                        ...
                                        if (zone.polygon.length < 3) {
                                                throw new Error(`Zone ${index + 1} needs at least 3 polygon points.`);
                                        }
        ...
        async function handleChallengeImageFileChange(event: ChangeEvent<HTMLInputElement>) {
                const file = event.target.files?.[0];
                event.target.value = "";
        ...
        async function handleSaveChallenge(event: FormEvent<HTMLFormElement>) {
                ...
                try {
                        if (isScavengerHunt) {
                                checkpointInputs = challengeDraft.checkpoints.map(checkpointDraftToWriteInput);
                                ...
                                minAccuracyMeters = Number(challengeDraft.min_accuracy_meters.trim() || "50");
                                if (!Number.isFinite(minAccuracyMeters) || minAccuracyMeters <= 0) {
                                        throw new Error("Minimum GPS accuracy must be greater than 0.");
                                }
                                requiredDwellSeconds = Number(challengeDraft.required_dwell_seconds.trim() || "30");
                                if (!Number.isFinite(requiredDwellSeconds) || requiredDwellSeconds <= 0) {
                                        throw new Error("Required visit time must be greater than 0 seconds.");
                                }
                                grandPrizePoints = Number(challengeDraft.grand_prize_points.trim() || "0");
                        ...
                        startTime = parseDateTimeLocalInput(challengeDraft.start_time, "Challenge start");
                        ...
                        if (
                                ...
                                if (!Number.isFinite(repeatCount) || repeatCount < 2 || repeatCount > 52) {
                        ...
                        const savedChallenges = challengeDraft.challenge_uuid
                                ...
                                : getCreatedChallenges(
                                                await createSchoolChallenge(
                                                        context.managedAppId,
                                                        activeSchoolId,
                                                        !isScavengerHunt && challengeDraft.repeat_enabled && repeatCount > 1
                                                                ? {
                                                                                ...payload,
                                                                                repeat: {
        ...
        async function handleDeleteSelectedChallenge() {
                ...
                try {
                        ...
                        setSchoolChallenges((current) =>
                                current.filter(
                                        (challenge) => challenge.challenge_uuid !== selectedChallenge.challenge_uuid,
                                ),
                        );
                        setChallengeParticipants([]);
                        setSelectedChallengeId("");
        ...
        async function createPackFromDraft(
                ...
                try {
                        const campusId =
                                draft.campus_id.trim() || schoolDraft.default_campus_id.trim() || undefined;
