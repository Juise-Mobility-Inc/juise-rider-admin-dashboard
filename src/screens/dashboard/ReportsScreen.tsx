...
type ReportTrendPoint = {
  ...
  ridePenalties: number;
  poiVisits: number;
  parkingViolations: number;
};

type ReportsVisuals = {
  deviceTypes: ReportChartItem[];
  penaltyTypes: ReportChartItem[];
  topPois: ReportChartItem[];
  studentsToReview: ReportChartItem[];
  reservationStatuses: ReportChartItem[];
...
function normalizeReportLabel(value: string | undefined, fallback: string): string {
...
function incrementCount(
  ...
  counts.set(label, (counts.get(label) ?? 0) + incrementBy);
}

function chartItemsFromCounts(
  counts: Map<string, number>,
  limit = 8,
): ReportChartItem[] {
...
function getMonthBucket(timestamp: number): { key: string; label: string } | null {
...
function getTrendPoint(
  trendByMonth: Map<string, ReportTrendPoint>,
  timestamp: number,
): ReportTrendPoint | null {
  const bucket = getMonthBucket(timestamp);
  if (!bucket) {
    return null;
  }

  const current = trendByMonth.get(bucket.key);
  if (current) {
    return current;
  }

  const nextPoint: ReportTrendPoint = {
    key: bucket.key,
    label: bucket.label,
    routeSessions: 0,
    ridePenalties: 0,
  ...
  trendByMonth.set(bucket.key, nextPoint);
...
function buildReportsVisuals(
  dataset: ReportsDataset,
  range: DateRange,
): ReportsVisuals {
  const deviceTypeCounts = new Map<string, number>();
  const penaltyTypeCounts = new Map<string, number>();
  const reservationStatusCounts = new Map<string, number>();
  const studentReviewCounts = new Map<string, ReportChartItem>();
  const trendByMonth = new Map<string, ReportTrendPoint>();
  const poiStatsByUUID = new Map<
    string,
    {
      title: string;
      visits: number;
  ...
  for (const bundle of dataset.students) {
    ...
    const studentName = formatStudentName(bundle.entry);
    const studentIdentifier =
      bundle.entry.membership.student_id.trim() || bundle.entry.user.email;
    const devices = bundle.profile?.devices ?? [];
    const routeSessions = getFilteredRouteSessions(bundle, range);
    const poiVisits = getFilteredPoiVisits(bundle, range);
    const penalties = getFilteredPenaltyEvents(bundle, range);
    const parkingViolations = getFilteredParkingViolations(bundle, range);

    for (const device of devices) {
      incrementCount(
        deviceTypeCounts,
        normalizeReportLabel(device.device_type, "Unknown device"),
      );
    }

    for (const session of routeSessions) {
      const trendPoint = getTrendPoint(trendByMonth, session.started_at);
      if (trendPoint) {
        trendPoint.routeSessions += 1;
      }
    }

    for (const { poi } of poiVisits) {
      const trendPoint = getTrendPoint(trendByMonth, poi.visited_at);
      if (trendPoint) {
        trendPoint.poiVisits += 1;
      }

      const poiKey = poi.poi_uuid.trim() || poi.title.trim();
      if (!poiKey) {
        continue;
      }

      const current = poiStatsByUUID.get(poiKey);
      if (current) {
        current.visits += 1;
        current.uniqueStudents.add(studentUserUUID);
      } else {
        poiStatsByUUID.set(poiKey, {
          title: normalizeReportLabel(poi.title, "Untitled POI"),
          visits: 1,
          uniqueStudents: new Set([studentUserUUID]),
        });
      }
    }

    for (const { event } of penalties) {
      const trendPoint = getTrendPoint(trendByMonth, event.occurred_at);
      if (trendPoint) {
        trendPoint.ridePenalties += 1;
      }
      incrementCount(penaltyTypeCounts, formatZoneType(event.zone_type));
    }

    for (const violation of parkingViolations) {
      const trendPoint = getTrendPoint(trendByMonth, violation.created_at);
      if (trendPoint) {
        trendPoint.parkingViolations += 1;
      }
    }

    const reviewCount = penalties.length + parkingViolations.length;
    if (reviewCount > 0) {
      studentReviewCounts.set(studentUserUUID, {
        label: studentName,
        value: reviewCount,
  ...
  for (const reservation of getFilteredReservations(dataset.reservations, range)) {
    incrementCount(
      reservationStatusCounts,
      normalizeReportLabel(reservation.status, "Unknown status"),
    );
  }

  return {
    deviceTypes: chartItemsFromCounts(deviceTypeCounts),
    penaltyTypes: chartItemsFromCounts(penaltyTypeCounts),
    topPois: Array.from(poiStatsByUUID.values())
      .map((stats) => ({
        label: stats.title,
        value: stats.visits,
        detail: `${stats.uniqueStudents.size} students`,
      }))
      .sort((left, right) => {
        if (left.value !== right.value) {
          return right.value - left.value;
      ...
      .slice(0, 8),
    studentsToReview: Array.from(studentReviewCounts.values())
      .sort((left, right) => {
        if (left.value !== right.value) {
          return right.value - left.value;
        }
      ...
      .slice(0, 8),
    reservationStatuses: chartItemsFromCounts(reservationStatusCounts),
...
function ReportBarVisualizer({
  ...
  return (
    <article className="reports-visual-card">
      <div className="reports-visual-heading">
        <h3>{title}</h3>
        <p>{description}</p>
      </div>
      {items.length === 0 ? (
        <p className="reports-visual-empty">{emptyLabel}</p>
      ) : (
          ...
          {items.map((item) => {
            ...
            return (
              <div className="reports-bar-row" key={item.label}>
                ...
                {item.detail ? (
                  <span className="reports-bar-detail">{item.detail}</span>
                ) : null}
              </div>
            );
          })}
...
function ReportTrendVisualizer({ points }: { points: ReportTrendPoint[] }) {
  const maxValue = Math.max(
    ...
    ...points.map(
      (point) =>
        ...
        point.ridePenalties +
        point.poiVisits +
        point.parkingViolations,
    ),
  );

  return (
    <article className="reports-visual-card reports-visual-card-wide">
      <div className="reports-visual-heading reports-visual-heading-row">
        <div>
          <h3>Activity trend</h3>
          <p>Route sessions, POI visits, ride penalties, and parking violations by month.</p>
        </div>
        <div className="reports-trend-legend">
          <span className="reports-trend-key reports-trend-key-sessions">Sessions</span>
          <span className="reports-trend-key reports-trend-key-pois">POIs</span>
          <span className="reports-trend-key reports-trend-key-ride">Ride penalties</span>
          <span className="reports-trend-key reports-trend-key-parking">Parking</span>
        </div>
      </div>
      {points.length === 0 ? (
        <p className="reports-visual-empty">No dated activity in this report period.</p>
      ) : (
        <div className="reports-trend-list">
          {points.map((point) => {
            ...
            return (
              <div className="reports-trend-row" key={point.key}>
                ...
                <div className="reports-trend-track">
                  <div
                    ...
                    style={{ width: `${width}%` }}
                  >
                    {point.routeSessions > 0 ? (
                      <span
                        className="reports-trend-segment reports-trend-segment-sessions"
                        style={{ flex: point.routeSessions }}
                      />
                    ) : null}
                    {point.poiVisits > 0 ? (
                      <span
                        className="reports-trend-segment reports-trend-segment-pois"
                        style={{ flex: point.poiVisits }}
                      />
                    ) : null}
                    {point.ridePenalties > 0 ? (
                      <span
                        className="reports-trend-segment reports-trend-segment-ride"
                        style={{ flex: point.ridePenalties }}
                      />
                    ) : null}
                    {point.parkingViolations > 0 ? (
                      <span
                        className="reports-trend-segment reports-trend-segment-parking"
                        style={{ flex: point.parkingViolations }}
                      />
                    ) : null}
                  </div>
                </div>
...
export function ReportsScreen({
  ...
  const reportVisuals = useMemo(
    () => (dataset ? buildReportsVisuals(dataset, dateRange) : null),
    [dataset, dateRange],
  );
    ...
    <section className="panel reports-section">
      ...
      {reportVisuals ? (
        <div className="reports-visual-grid">
          <ReportTrendVisualizer points={reportVisuals.activityTrend} />
          <ReportBarVisualizer
            title="Device mix"
            description="Current registered device types across the roster."
            items={reportVisuals.deviceTypes}
            emptyLabel="No registered devices loaded."
          />
          <ReportBarVisualizer
            title="Ride penalty types"
            description="Speed-limit and no-go events in the selected report period."
            items={reportVisuals.penaltyTypes}
            emptyLabel="No ride penalties in this period."
          />
          <ReportBarVisualizer
            title="Top POIs"
            description="Most visited points of interest and their student reach."
            items={reportVisuals.topPois}
            emptyLabel="No POI visits in this period."
          />
          <ReportBarVisualizer
            title="Students to review"
            description="Students with the highest combined ride penalties and parking violations."
            items={reportVisuals.studentsToReview}
            emptyLabel="No penalty or violation records in this period."
          />
          <ReportBarVisualizer
            title="Reservation status"
            description="Parking reservation outcomes in the selected report period."
            items={reportVisuals.reservationStatuses}
            emptyLabel="No reservations in this period."
          />
        </div>
      ) : null}
