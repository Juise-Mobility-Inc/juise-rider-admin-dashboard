...
function fullName(e: SchoolStudentRosterEntry): string {
  const n = `${e.user.first_name?.trim() ?? ""} ${e.user.last_name?.trim() ?? ""}`.trim();
...
function fmtFull(ts: number): string {
  if (!ts) return "—";
  return new Date(ts * 1000).toLocaleString([], { weekday: "short", month: "short", day: "numeric", year: "numeric", hour: "numeric", minute: "2-digit" });
}
...
export function StudentRoutesScreen({ activeSchoolId, managedAppId }: Props) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      if (dead) return;
      setRoster(r); setPOIs(p); setZones(z);
    }).catch((e) => {
      if (!dead) setRosterErr(e instanceof Error ? e.message : "Failed to load");
    }).finally(() => { if (!dead) setRosterBusy(false); });
    return () => { dead = true; };
  ...
  const filtered = useMemo(() => {
    ...
    if (!q) return roster;
    return roster.filter((e) => fullName(e).toLowerCase().includes(q) || subline(e).toLowerCase().includes(q));
  }, [roster, search]);

  const selEntry = useMemo(() => roster.find((e) => uuid(e) === selUUID) ?? null, [roster, selUUID]);
  const selSess = useMemo(() => history.find((s) => s.session_id === selId) ?? null, [history, selId]);
  ...
  const segments = useMemo((): Seg[] => {
    ...
    for (let i = 1; i < pts.length; i++) {
      ...
      if (c !== col || i === pts.length - 1) {
        ...
        col = c; pos = [[pts[i].latitude, pts[i].longitude]];
      }
    }
    return out;
  }, [selSess, showRoute]);
  ...
  const routePts = useMemo((): [number, number][] => {
    if (!selSess) return [];
    return selSess.points.map((p): [number, number] => [p.latitude, p.longitude]);
  }, [selSess]);
          ...
          <div className="sr-student-list">
            {rosterBusy && (
              <div className="sr-skel-list">
                {[...Array(5)].map((_, i) => <div key={i} className="sr-skel-row" />)}
              </div>
            )}
            {!rosterBusy && filtered.length === 0 && !rosterErr && (
              <p className="sr-empty-hint">{search ? "No match." : "No students."}</p>
            )}
            {!rosterBusy && filtered.map((entry) => {
              const id = uuid(entry);
              ...
              return (
                ...
                >
                  <div className="sr-avatar">
                    <span className="sr-avatar-initials">{initials(entry)}</span>
        ...
        <div className="sr-map-panel">
          <MapContainer center={DEFAULT_CENTER} zoom={DEFAULT_ZOOM} className="sr-map" zoomControl={true}>
            ...
            {showZones && dispZones.filter((z) => z.active && z.polygon.length >= 3).map((z) => (
              <Polygon
                key={z.zone_uuid}
                positions={z.polygon.map((p): [number, number] => [p.lat, p.lng])}
                pathOptions={{
                  color: z.zone_type === "no_go" ? "#e53e3e" : "#f6ae2d",
                  fillColor: z.zone_type === "no_go" ? "#e53e3e" : "#f6ae2d",
                  fillOpacity: 0.12, weight: 2,
                  dashArray: z.zone_type === "speed_limit" ? "6 4" : undefined,
                }}
              >
                <Popup>
                  <strong>{z.title || (z.zone_type === "no_go" ? "No-go zone" : "Speed limit zone")}</strong>
                  {z.description && <><br />{z.description}</>}
                  {z.zone_type === "speed_limit" && z.speed_limit_mph != null && <><br />Limit: {z.speed_limit_mph} mph</>}
                </Popup>
              </Polygon>
            ))}
            ...
            {showRoute && selSess && selSess.points.length > 0 && (<>
              <CircleMarker center={[selSess.points[0].latitude, selSess.points[0].longitude]} radius={9}
                pathOptions={{ color: "#fff", fillColor: "#27cc5e", fillOpacity: 1, weight: 2.5 }}>
                <Popup><strong>Ride started</strong><br />{fmtFull(selSess.started_at)}</Popup>
              </CircleMarker>
              <CircleMarker
                center={[selSess.points[selSess.points.length - 1].latitude, selSess.points[selSess.points.length - 1].longitude]}
                radius={9} pathOptions={{ color: "#fff", fillColor: "#112d4e", fillOpacity: 1, weight: 2.5 }}>
                <Popup><strong>Ride ended</strong><br />{selSess.ended_at ? fmtFull(selSess.ended_at) : "—"}</Popup>
              </CircleMarker>
            </>)}

            {showPOIs && selSess?.visited_pois.map((poi, i) => (
              <CircleMarker key={`poi-${i}`} center={[poi.lat, poi.lng]} radius={10}
                pathOptions={{ color: "#c87a00", fillColor: "#f6ae2d", fillOpacity: 0.95, weight: 2 }}>
                <Popup>
                  <strong>{poi.title || "Point of Interest"}</strong><br />+{poi.bonus_points} pts
                  {poi.description && <><br />{poi.description}</>}
                  {poi.visited_at > 0 && <><br /><span style={{ color: "#888", fontSize: "0.82em" }}>{fmtShort(poi.visited_at)}</span></>}
                </Popup>
              </CircleMarker>
            ))}

            {showPenalties && selSess?.penalty_events.map((ev, i) => (
              <CircleMarker key={`pen-${i}`} center={[ev.lat, ev.lng]} radius={10}
                pathOptions={{ color: "#9b1c1c", fillColor: "#e53e3e", fillOpacity: 0.95, weight: 2 }}>
                <Popup>
                  <strong>{ev.title || (ev.zone_type === "no_go" ? "No-go zone" : "Speed limit zone")}</strong><br />
                  −{ev.points_lost} pts
                  {ev.reason && <><br />{ev.reason}</>}
                  {ev.zone_type === "speed_limit" && ev.speed_limit_mph != null && <><br />Limit: {ev.speed_limit_mph} mph</>}
                  {ev.duration_ms > 0 && <><br />Duration: {Math.round(ev.duration_ms / 1000)}s</>}
                </Popup>
              </CircleMarker>
            ))}
          </MapContainer>
          ...
          {hasGPS && (
            <div className="sr-overlay-filters">
              ...
              {layerToggles.map((f) => (
                <button key={f.key} className={`sr-filter-btn${f.active ? " sr-filter-btn--on" : ""}`} onClick={f.toggle}>
              ...
              {showRoute && (
                <div className="sr-speed-legend">
                  {[
                    { l: "< 5 mph", c: "#27cc5e" }, { l: "5–10", c: "#a8d63c" },
                    { l: "10–15", c: "#f6ae2d" }, { l: "15–20", c: "#ff6b35" }, { l: "20+", c: "#e53e3e" },
                  ].map((s) => (
                    <span key={s.l} className="sr-swatch">
                      <span className="sr-swatch-dot" style={{ background: s.c }} />{s.l}
                    </span>
                  ))}
                </div>
              )}
          ...
          {selSess && (
            <div className={`sr-overlay-stats${statsOpen ? " sr-overlay-stats--open" : ""}`}>
              <button className="sr-stats-hd" onClick={() => setStatsOpen((v) => !v)}>
                <span className="sr-stats-hd-title">{fmtShort(selSess.started_at)}</span>
                <span className="sr-stats-hd-icon">{statsOpen ? "▾" : "▴"}</span>
              </button>
              {statsOpen && (
                <div className="sr-stats-grid">
                  {[
                    { label: "Distance", value: fmtDist(selSess.distance_meters) },
                    { label: "Duration", value: fmtDur(selSess.duration_seconds) },
                    { label: "Points", value: `+${selSess.bonus_points}`, cls: "sr-val--green" },
                    ...(selSess.penalty_points > 0 ? [{ label: "Penalties", value: `−${selSess.penalty_points}`, cls: "sr-val--red" }] : []),
                    { label: "POI visits", value: String(selSess.visited_pois.length) },
                    { label: "Top speed", value: `${(selSess.top_speed_mps * 2.237).toFixed(1)} mph` },
                    { label: "GPS pts", value: selSess.points.length.toLocaleString() },
                  ].map((s) => (
                    <div key={s.label} className="sr-stat">
                      <span className="sr-stat-label">{s.label}</span>
                      <strong className={`sr-stat-val${s.cls ? ` ${s.cls}` : ""}`}>{s.value}</strong>
                    </div>
                  ))}
                </div>
              )}
              ...
              <div className="sr-map-msg">
                <div className="sr-map-msg-icon">
                  <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
                    <circle cx="12" cy="11" r="3" /><path d="M17.657 16.657L13.414 20.9a2 2 0 01-2.828 0l-4.243-4.243a8 8 0 1111.314 0z" />
                  </svg>
                </div>
                <p className="sr-map-msg-title">Select a student</p>
                <p className="sr-map-msg-desc">Choose a student from the left sidebar to view their ride history on the map.</p>
              </div>
            </div>
          )}
          {selUUID && histBusy && (
            <div className="sr-map-overlay">
              <div className="sr-map-msg"><p className="sr-map-msg-title">Loading rides…</p></div>
            </div>
          )}
          {noRides && (
            <div className="sr-map-overlay">
              <div className="sr-map-msg">
                ...
                <p className="sr-map-msg-desc">This student has no route history yet.</p>
              </div>
            </div>
          )}
          {noGPS && !noRides && (
            <div className="sr-map-overlay">
              <div className="sr-map-msg">
                ...
                <p className="sr-map-msg-desc">This ride was manually logged — no GPS breadcrumbs available.</p>
              </div>
            </div>
          )}
        </div>
      ...
      {selUUID && (
        <div className="sr-ride-bar">
          <div className="sr-ride-bar-head">
            ...
            {!histBusy && history.length > 0 && <span className="sr-count">{history.length}</span>}
            {histErr && <span className="sr-err-inline">{histErr}</span>}
            {histBusy && <span className="sr-muted-inline">Loading…</span>}
          </div>

          {noRides && !histBusy && (
            <p className="sr-ride-bar-empty">No rides recorded for this student yet.</p>
          )}
              ...
              {history.map((sess) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className="sr-chip-dur">{fmtDur(sess.duration_seconds)}</span>
                    <span className="sr-chip-badges">
                      {sess.bonus_points > 0 && (
                        <span className="sr-badge sr-badge--green">+{sess.bonus_points}</span>
                      )}
                      {sess.penalty_events.length > 0 && (
                        <span className="sr-badge sr-badge--red">{sess.penalty_events.length}⚠</span>
                      )}
                      {sess.visited_pois.length > 0 && (
                        <span className="sr-badge sr-badge--gold">{sess.visited_pois.length}★</span>
                      )}
